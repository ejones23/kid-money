...
- Private CloudKit family sharing is explicitly requested and approved for Phase
  ...
  passed. Do not wire real-ledger upload into the app until participant adoption,
  initial fetch, and activation failure recovery are covered by injected tests.
...
`docs/FAMILY_SHARING_DESIGN.md`. Add a dormant participant invitation-adoption
coordinator: validate the share/container and empty-ledger preflight, accept the
share through an injected transport, persist the shared-zone identity, perform
the initial scoped fetch/merge, and recover interruption without wiring it into
the app. Preserve the physically verified Siri grammar.
