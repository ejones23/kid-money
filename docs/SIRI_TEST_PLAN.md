...
- Next discriminators: first update the managed phone from iOS 26.6.1 to 26.6.2 through its normal company-approved update path, if offered, and retest. If routing still fails, delete the test app and install build 3 fresh, accepting that this erases its local test ledger; record whether the one-time shortcut authorization prompt appears. If a fresh install still fails, inspect management-visible Siri settings with IT rather than installing diagnostic profiles unilaterally.
